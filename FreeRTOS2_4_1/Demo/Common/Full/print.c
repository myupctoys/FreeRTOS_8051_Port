...
{
const unsigned portCHAR ucQueueSize = ( unsigned portCHAR ) 20;

	/* Create the queue on which errors will be reported. */
	xPrintQueue = xQueueCreate( ucQueueSize, ( unsigned portCHAR ) sizeof( portCHAR * ) );
}
/*-----------------------------------------------------------*/
...
{
	#ifdef USE_STDIO
		cQueueSend( xPrintQueue, ( void * ) ppcMessageToSend, ( portTickType ) 0 );
	#else
    	/* Stop warnings. */
...
portCHAR *pcMessage;
	...
	if( cQueueReceive( xPrintQueue, &pcMessage, xPrintRate ) == pdPASS )
	{
		return pcMessage;
	}
	else
