...
Changes from V2.1.1
	...
	+ The stack size now uses portMINIMAL_STACK_SIZE.
...
#define semtstSTACK_SIZE			portMINIMAL_STACK_SIZE
...
void vStartSemaphoreTasks( unsigned portCHAR ucPriority )
{
	...
	if( pxFirstSemaphoreParameters != NULL )
	{
		...
		if( pxFirstSemaphoreParameters->xSemaphore != NULL )
		{
			...
			*( pxFirstSemaphoreParameters->pulSharedVariable ) = semtstNON_BLOCKING_EXPECTED_VALUE;
			...
			sTaskCreate( prvSemaphoreTest, "PolSEM1", semtstSTACK_SIZE, ( void * ) pxFirstSemaphoreParameters, tskIDLE_PRIORITY, ( xTaskHandle * ) NULL );
			sTaskCreate( prvSemaphoreTest, "PolSEM2", semtstSTACK_SIZE, ( void * ) pxFirstSemaphoreParameters, tskIDLE_PRIORITY, ( xTaskHandle * ) NULL );
		}
	}
	...
	if( pxSecondSemaphoreParameters != NULL )
	{
		...
		if( pxSecondSemaphoreParameters->xSemaphore != NULL )
		{
			...
			*( pxSecondSemaphoreParameters->pulSharedVariable ) = semtstBLOCKING_EXPECTED_VALUE;
			pxSecondSemaphoreParameters->xBlockTime = xBlockTime / portTICKS_PER_MS;

			sTaskCreate( prvSemaphoreTest, "BlkSEM1", semtstSTACK_SIZE, ( void * ) pxSecondSemaphoreParameters, ucPriority, ( xTaskHandle * ) NULL );
			sTaskCreate( prvSemaphoreTest, "BlkSEM2", semtstSTACK_SIZE, ( void * ) pxSecondSemaphoreParameters, ucPriority, ( xTaskHandle * ) NULL );
		}
	}
}
/*-----------------------------------------------------------*/
...
portSHORT sError = pdFALSE, sCheckVariableToUse;
	...
	for( ;; )
	{
		...
		if( cSemaphoreTake( pxParameters->xSemaphore, pxParameters->xBlockTime ) == pdPASS )
		{
			...
			variable. */
			if(	cSemaphoreGive( pxParameters->xSemaphore ) == pdFALSE )
			{
				vPrintDisplayMessage( &pcPollingSemaphoreTaskError );
				sError = pdTRUE;
			}
...
portSHORT sAreSemaphoreTasksStillRunning( void )
{
static portSHORT sLastCheckVariables[ semtstNUM_TASKS ] = { 0 };
portSHORT sTask, sReturn = pdTRUE;

	for( sTask = 0; sTask < semtstNUM_TASKS; sTask++ )
	{
		if( sLastCheckVariables[ sTask ] == sCheckVariables[ sTask ] )
		{
			sReturn = pdFALSE;
		}

		sLastCheckVariables[ sTask ] = sCheckVariables[ sTask ];
	}
