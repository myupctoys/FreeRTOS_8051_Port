...
#define comRX_RELATIVE_PRIORITY		( ( unsigned portCHAR ) 1 )
...
void vStartComTestTasks( unsigned portCHAR ucPriority, eCOMPort ePort, eBaud eBaudRate )
{
const unsigned portCHAR ucBufferLength = ( unsigned portCHAR ) 255;

	/* Initialise the com port then spawn both tasks. */
	xPort = xSerialPortInit( ePort, eBaudRate, serNO_PARITY, serBITS_8, serSTOP_1, ucBufferLength );
	sTaskCreate( vComTxTask, "COMTx", comSTACK_SIZE, NULL, ucPriority, NULL );
	sTaskCreate( vComRxTask, "COMRx", comSTACK_SIZE, NULL, ucPriority + comRX_RELATIVE_PRIORITY, NULL );
	sTaskCreate( vSemTestTask, "ISRSemTst", comSTACK_SIZE, NULL, tskIDLE_PRIORITY, NULL );
}
/*-----------------------------------------------------------*/
...
const portTickType xBlockTime = ( portTickType ) 0xffff / portTICKS_PER_MS;
const portCHAR *pcExpectedChar;
portCHAR cRxedChar, cGotChar;
portSHORT sResyncRequired, sConsecutiveErrors, sLatchedError;
	...
	for( ;; )
	{
		...
		cGotChar = cSerialGetChar( xPort, &cRxedChar, xBlockTime );
		if( cGotChar == ( portCHAR ) pdTRUE )
		{
...
const portCHAR * const pcTaskStartMsg = "ISR Semaphore test started.\r\n";
	...
	for( ;; )
	{
		if( sSerialWaitForSemaphore( xPort ) )
		{
			sSemCount++;				
		}
	}
