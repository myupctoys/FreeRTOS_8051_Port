...
const unsigned portCHAR ucQueueSize = ( unsigned portCHAR ) 10;
	...
	xPolledQueue = xQueueCreate( ucQueueSize, ( unsigned portCHAR ) sizeof( unsigned portSHORT ) );

	/* Spawn the producer and consumer. */
	sTaskCreate( vPolledQueueConsumer, "QConsNB", pollqSTACK_SIZE, ( void * ) &xPolledQueue, ucPriority, NULL );
	sTaskCreate( vPolledQueueProducer, "QProdNB", pollqSTACK_SIZE, ( void * ) &xPolledQueue, ucPriority, NULL );
}
/*-----------------------------------------------------------*/
...
const portTickType xDelay = ( portTickType ) 200 / portTICKS_PER_MS;
...
portSHORT sError = pdFALSE;
	...
	{		
		for( usLoop = 0; usLoop < usNumToProduce; ++usLoop )
		{
			...
			if( cQueueSend( *pxQueue, ( void * ) &usValue, ( portTickType ) 0 ) != pdPASS )
			{
...
portSHORT sError = pdFALSE;
	...
	{		
		...
		while( ucQueueMessagesWaiting( *pxQueue ) )
		{
			if( cQueueReceive( *pxQueue, &usData, ( portTickType ) 0 ) == pdPASS )
			{
