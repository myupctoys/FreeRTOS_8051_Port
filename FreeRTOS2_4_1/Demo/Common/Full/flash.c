...
{
	unsigned portCHAR ucLED;		/*< The output the task should use. */
...
const portSHORT sNumOfLEDs = 8, sFlashRate = 125;

	/* Create the eight tasks. */
	for( sLEDTask = 0; sLEDTask < sNumOfLEDs; ++sLEDTask )
	{
		...
		pxLEDParameters->ucLED = ( unsigned portCHAR ) sLEDTask;
		pxLEDParameters->xFlashRate = ( portTickType ) ( sFlashRate + ( sFlashRate * sLEDTask ) );
		pxLEDParameters->xFlashRate /= portTICKS_PER_MS;

		/* Spawn the task. */
		sTaskCreate( vLEDFlashTask, "LEDx", ledSTACK_SIZE, ( void * ) pxLEDParameters, ucPriority, ( xTaskHandle * ) NULL );
	}
}
/*-----------------------------------------------------------*/
...
xLEDParameters *pxParameters;
	...
	for(;;)
	{
		...
		vParTestToggleLED( pxParameters->ucLED );
