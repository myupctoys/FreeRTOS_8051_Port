...
Changes from V1.2.3
	...
	+ ucTaskCheck is now just used as a boolean.  Instead of incrementing
...
#define intgCONST2				( ( portLONG ) 23456789 )
#define intgCONST3				( ( portLONG ) -9 )
#define intgCONST4				( ( portLONG ) 7 )
#define intgEXPECTED_ANSWER		( ( ( intgCONST1 + intgCONST2 ) * intgCONST3 ) / intgCONST4 )

#define intgSTACK_SIZE			portMINIMAL_STACK_SIZE
...
static void vCompeteingIntMathTask( void *pvParameters );

/* Variables that are set to true within the calculation task to indicate
that the task is still executing.  The check task sets the variable back to
false, flagging an error if the variable is still false the next time it
is called. */
static volatile signed portCHAR cTaskCheck[ intgNUMBER_OF_TASKS ] = { ( signed portCHAR ) pdFALSE };
...
portSHORT sTask;
	...
	{
		sTaskCreate( vCompeteingIntMathTask, ( const portCHAR * const ) "IntMath", intgSTACK_SIZE, ( void * ) &( cTaskCheck[ sTask ] ), ucPriority, ( xTaskHandle * ) NULL );
	}
}
/*-----------------------------------------------------------*/
...
volatile signed portCHAR *pcTaskHasExecuted;

	/* Set a pointer to the variable we are going to set to true each
	iteration.  This is also a good test of the parameter passing mechanism
	within each port. */
	pcTaskHasExecuted = ( volatile signed portCHAR * ) pvParameters;
	...
	for( ;; )
	{
		...
		if( sError == pdFALSE )
		{
			...
			the check task. */
			portENTER_CRITICAL();
				*pcTaskHasExecuted = ( unsigned portCHAR ) pdTRUE;
			portEXIT_CRITICAL();
		}
...
{
volatile portSHORT sReturn = pdTRUE, sTask;

	/* Check the maths tasks are still running by ensuring their check variables 
	are still being set to true. */
	for( sTask = 0; sTask < intgNUMBER_OF_TASKS; sTask++ )
	{
		if( cTaskCheck[ sTask ] == ( portCHAR ) pdFALSE )
		{
		...
		cTaskCheck[ sTask ] = ( portCHAR ) pdFALSE;
	}
