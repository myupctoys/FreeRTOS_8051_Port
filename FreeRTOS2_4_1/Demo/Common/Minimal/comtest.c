...
#define comSTACK_SIZE				portMINIMAL_STACK_SIZE
#define comTX_LED_OFFSET			( ( unsigned portCHAR ) 0 )
#define comRX_LED_OFFSET			( ( unsigned portCHAR ) 1 )
...
#define comBUFFER_LEN				( ( unsigned portCHAR ) ( comLAST_BYTE - comFIRST_BYTE ) + ( unsigned portCHAR ) 1 )
#define comINITIAL_RX_COUNT_VALUE	( ( unsigned portCHAR ) 0 )
...
toggle LED ( ucBaseLED + comRX_LED_OFFSET).  The Tx task will toggle LED
( ucBaseLED + comTX_LED_OFFSET ). */
static unsigned portCHAR ucBaseLED = ( unsigned portCHAR ) 0;

/* Check variable used to ensure no error have occurred.  The Rx task will 
increment this variable after every successfully received sequence.  If at any
time the sequence is incorrect the the variable will stop being incremented. */
static volatile unsigned portCHAR ucRxLoops = comINITIAL_RX_COUNT_VALUE;

/*-----------------------------------------------------------*/

void vAltStartComTestTasks( unsigned portCHAR ucPriority, unsigned portLONG ulBaudRate, unsigned portCHAR ucLED )
{
	...
	sTaskCreate( vComTxTask, ( const signed portCHAR * const ) "COMTx", comSTACK_SIZE, NULL, ucPriority - ( unsigned portCHAR ) 1, ( xTaskHandle * ) NULL );
	sTaskCreate( vComRxTask, ( const signed portCHAR * const ) "COMRx", comSTACK_SIZE, NULL, ucPriority, ( xTaskHandle * ) NULL );
}
/*-----------------------------------------------------------*/
...
portTickType xTimeToWait;
	...
	for( ;; )
	{
		...
		{
			if( cSerialPutChar( xPort, cByteToSend, comNO_BLOCK ) == ( signed portCHAR ) pdPASS )
			{
				vParTestToggleLED( ucBaseLED + comTX_LED_OFFSET );
			}
		}

		/* Turn the LED off while we are not doing anything. */
		vParTestSetLED( ucBaseLED + comTX_LED_OFFSET, pdFALSE );
...
static void vComRxTask( void *pvParameters )
{
signed portCHAR cExpectedByte, cByteRxed, cResyncRequired = ( signed portCHAR ) pdFALSE, cErrorOccurred = ( signed portCHAR ) pdFALSE;
	...
	for( ;; )
	{
		...
		for( cExpectedByte = comFIRST_BYTE; cExpectedByte <= comLAST_BYTE; cExpectedByte++ )
		{
			...
			if( cSerialGetChar( xPort, &cByteRxed, comRX_BLOCK_TIME ) )
			{
				...
				{
					vParTestToggleLED( ucBaseLED + comRX_LED_OFFSET );
				}
				else
				{
					cResyncRequired = ( signed portCHAR ) pdTRUE;
		...
		/* Turn the LED off while we are not doing anything. */
		vParTestSetLED( ucBaseLED + comRX_LED_OFFSET, pdFALSE );

		/* Did we break out of the loop because the characters were received in
		an unexpected order?  If so wait here until the character sequence is 
		about to restart. */
		if( cResyncRequired == ( signed portCHAR ) pdTRUE )
		{
			while( cByteRxed != comLAST_BYTE )
			{
				/* Block until the next char is available. */
				cSerialGetChar( xPort, &cByteRxed, comRX_BLOCK_TIME );
			}
			...
			error. */
			cErrorOccurred = ( signed portCHAR ) pdTRUE;

			/* We have now resynced with the Tx task and can continue. */
			cResyncRequired = ( signed portCHAR ) pdFALSE;
		}
		else
		{
			if( cErrorOccurred == ( signed portCHAR ) pdFALSE )
			{
...
portSHORT sAreComTestTasksStillRunning( void )
{
portSHORT sReturn;
	...
	if( ucRxLoops == comINITIAL_RX_COUNT_VALUE )
	{
		sReturn = pdFALSE;
	}
	else
	...
	ucRxLoops = comINITIAL_RX_COUNT_VALUE;
